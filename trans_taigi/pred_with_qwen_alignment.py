...
SYSTEM_PROMPT = """繁體中文回答，你是一個翻譯錯誤分析助手。請根據以下「原文（中文）」與「機器翻譯（台語漢字/台文）」進行錯誤偵測與**文字內嵌標記**，並以**單一 JSON 物件**輸出結果。
...
def main():
    ...
    output_path = f"./pred_output/{MODEL_NAME}_random_sent7_word3.jsonl"
    ...
    # Build the two retrievers (multilingual-e5-base)
    sent_retriever = E5Retriever(df_sentence["中文"].astype(str).tolist(),
                                model_name="intfloat/multilingual-e5-base")
    word_retriever = E5Retriever(df_word["中文"].astype(str).tolist(),
                                model_name="intfloat/multilingual-e5-base")
    ...
    SENT_TOP_K = 7        # Number of sentence-alignment few-shots (also used for random sent)
    SENT_MIN_SCORE = 0.8     # Sentence similarity threshold (cosine; drop below this)

    WORD_PER_TOKEN_TOP = 3  # Max matched words per Chinese token
    ...
    for _, d in tqdm(data.iterrows(), total=len(data)):
        ...
        # error_spans, explanation, score = process_one_record(d)
